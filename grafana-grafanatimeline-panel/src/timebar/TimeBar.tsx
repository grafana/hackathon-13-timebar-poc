...
import { TimebarActions, useTimebar } from './timebarState';
...
const valToContainerPx = (over: HTMLElement, t: number, ctx: TimeRangeMs): number =>
  over.offsetLeft + ((t - ctx.from) / (ctx.to - ctx.from || 1)) * over.clientWidth;
...
const getStyles = (theme: GrafanaTheme2) => ({
  ...
  selectionBox: css`
    ...
    border-left: 1px solid ${theme.colors.primary.border};
...
export const TimeBar: React.FC<TimeBarProps> = ({
  ...
  const data = useMemo<uPlot.AlignedData>(() => [time, values], [time, values]);
  ...
  const updateOverlay = useCallback(() => {
    ...
    setOverlay({
      left: valToContainerPx(over, sel.from, ctx),
      right: valToContainerPx(over, sel.to, ctx),
      top: over.offsetTop,
  ...
  const builder = useMemo(() => {
    ...
    b.addHook('ready', (u: uPlot) => {
      ...
      const axis = u.root.querySelector('.u-axis') as HTMLElement | null;
      ...
      (u as unknown as { _timebarCleanup?: () => void })._timebarCleanup = () => {
  ...
  useEffect(() => {
    ...
    updateOverlay();
  }, [state.contextWindow.from, state.contextWindow.to, state.selection.from, state.selection.to, chartWidth, updateOverlay]);
  ...
  const startSelectionDrag = useCallback((e: React.MouseEvent, kind: 'move' | 'left' | 'right') => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      if (kind === 'move') {
        ...
        fromPx = Math.min(origFromPx + deltaPx, toPx - MIN_SELECTION_PX);
      } else {
        toPx = Math.max(origToPx + deltaPx, fromPx + MIN_SELECTION_PX);
      }
  ...
  return (
    <div className={styles.wrapper} style={{ width, height }} data-testid="timebar">
      <div className={styles.controlRow}>
        ...
        <IconButton tooltip="Pan left" name="arrow-left" onClick={() => actions.pan('left')} />
        <IconButton tooltip="Zoom out context" name="search-minus" onClick={() => actions.zoom(2)} />
        <IconButton tooltip="Zoom in context" name="search-plus" onClick={() => actions.zoom(0.5)} />
        <IconButton tooltip="Pan right" name="arrow-right" onClick={() => actions.pan('right')} />
        <IconButton tooltip="Reset context window" name="crosshair" onClick={() => actions.reset()} />
      </div>
        ...
        {overlay && (
          <>
            <div
              ...
              style={{ left: overlay.left, top: overlay.top, width: overlay.right - overlay.left, height: overlay.height }}
