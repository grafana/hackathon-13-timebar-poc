...
export function useTimebar({
  ...
  const zoom = useCallback((factor: number) => {
    dispatch({ type: 'setContextWindow', range: zoomRange(stateRef.current.contextWindow, factor), now: nowRef.current });
  }, []);

  const pan = useCallback((direction: 'left' | 'right') => {
    dispatch({ type: 'setContextWindow', range: panRange(stateRef.current.contextWindow, direction), now: nowRef.current });
  }, []);
